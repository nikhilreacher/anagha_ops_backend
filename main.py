...
def run_dispatch_migration():
    ...
    with engine.begin() as connection:
        ...
        if "payment_events" in table_names and "allocation_mode" not in payment_event_columns:
            connection.execute(text(f"ALTER TABLE {payment_events_table} ADD COLUMN IF NOT EXISTS allocation_mode VARCHAR DEFAULT 'oldest'"))
        if "payment_events" in table_names and "allocations_json" not in payment_event_columns:
            connection.execute(text(f"ALTER TABLE {payment_events_table} ADD COLUMN IF NOT EXISTS allocations_json VARCHAR"))
        if "payment_events" in table_names and "source" not in payment_event_columns:
            connection.execute(text(f"ALTER TABLE {payment_events_table} ADD COLUMN IF NOT EXISTS source VARCHAR DEFAULT 'direct'"))
        if "payment_events" in table_names and "source_request_id" not in payment_event_columns:
            connection.execute(text(f"ALTER TABLE {payment_events_table} ADD COLUMN IF NOT EXISTS source_request_id INTEGER"))
        if "payment_events" in table_names and "business_type" not in payment_event_columns:
            connection.execute(text(f"ALTER TABLE {payment_events_table} ADD COLUMN IF NOT EXISTS business_type VARCHAR DEFAULT 'mainline'"))
        if "payment_events" in table_names and "created_at" not in payment_event_columns:
            connection.execute(text(f"ALTER TABLE {payment_events_table} ADD COLUMN IF NOT EXISTS created_at TIMESTAMP"))
        ...
        if "payment_events" in table_names:
            connection.execute(
                text(
                    f"""
                    UPDATE {payment_events_table}
